...
- (void)gettingRequestTokenUsingOAuth
{
	...
	//NSLog(@"post[%@]", getRequestTokenHeadInfo);
	if( _request )
	{
		[_request release];
		_request = nil;
...
- (void)openUserAuthorizePage
{
	...
	NSString* urlString = [WBRequest serializeURL:oauthUserAuthorizeURL 
									   ...
									   httpMethod:@"GET"];
	//NSLog(@"the request url:%@", urlString);
    if ([_weibo.delegate respondsToSelector:@selector(openAuthPage:)]) {
        [_weibo.delegate openAuthPage:urlString];
    } else {
